...
- (void)setupCaptureSession {
    self.mCaptureSession = [AVCaptureSession new];
    ...
    [self.mCaptureDeviceOutput setAlwaysDiscardsLateVideoFrames:false];
    NSDictionary *settings = [NSDictionary dictionaryWithObject:[NSNumber numberWithInt:kCMPixelFormat_32BGRA]
                                                         forKey:(id)kCVPixelBufferPixelFormatTypeKey];
    [self.mCaptureDeviceOutput setVideoSettings:settings];
...
- (void)setupPipeline {
    ...
    id<MTLFunction> vertexFunction = [defaultLibrary newFunctionWithName:@"threadHoldVertexShader"]; // 顶点shader，vertexShader是函数名
    id<MTLFunction> fragmentFunction = [defaultLibrary newFunctionWithName:@"threadHoldSamplingShader"]; // 片元shader，samplingShader是函数名
    ...
    // 创建计算管道
    self.computePipelineState = [self.mtkView.device newComputePipelineStateWithFunction:kernelFunction error:NULL];
    // 获取渲染队列
...
- (void)setupTexture {
    ...
    textureDescriptor.usage = MTLResourceUsageRead | MTLTextureUsageShaderWrite;
...
- (void)captureOutput:(AVCaptureOutput *)output didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    size_t with = CVPixelBufferGetWidth(pixelBuffer);
    size_t heigt = CVPixelBufferGetHeight(pixelBuffer);
    CVMetalTextureRef tmpTexture = NULL;
    CVReturn status = CVMetalTextureCacheCreateTextureFromImage(kCFAllocatorDefault, self.textureCache, pixelBuffer, NULL, MTLPixelFormatBGRA8Unorm, with, heigt, 0, &tmpTexture);
    if (status == kCVReturnSuccess) {
        self.mtkView.drawableSize = CGSizeMake(with, heigt);
