...
//  LearnMetal
//
//  Created by Renjun Li on 2022/5/24.
...
@interface GrayViewController () <MTKViewDelegate>
...
-(void)setupPipeline {
    ...
    id<MTLFunction> vertexFunction = [defaultLibrary newFunctionWithName:@"grayVertexShader"]; // 顶点shader，vertexShader是函数名
    id<MTLFunction> fragmentFunction = [defaultLibrary newFunctionWithName:@"graySampling"];
    id<MTLFunction> kernerlFunction = [defaultLibrary newFunctionWithName:@"getGrayKernel"];
    ...
    self.renderPipelineState = [self.mtkView.device newRenderPipelineStateWithDescriptor:pipelineStateDescriptor
                                                                                   error:NULL];
    self.computePipelineState = [self.mtkView.device newComputePipelineStateWithFunction:kernerlFunction error:nil];
...
- (void)setupTexture {
    ...
    textureDescriptor.usage = MTLTextureUsageShaderWrite | MTLResourceUsageRead;
